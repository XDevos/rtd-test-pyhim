    ...
    if fileName in session1.data:
        ...
        if param.param["zProject"]["display"]:
            ...
            if param.param["zProject"]["mode"] == "laplacian":
                outputName = Im.getImageFileName(
                    dataFolder.outputFolders["zProject"], "_focalPlaneMatrix.png"
                )
                Im.imageShowWithValues(outputName)

                writeString2File(
                    param.param["fileNameMD"],
                    "{}\n ![]({})\n".format(os.path.basename(fileName), outputName),
                    "a",
                )  # initialises MD file
        ...
        if param.param["parallel"]:
            ...
            files2ProcessFiltered = [
                ...
                or (
                    fileName != None
                    and (
                        os.path.basename(x) in [os.path.basename(x1) for x1 in fileName]
                    )
                )
            ]
            ...
            if len(files2ProcessFiltered) > 0:
                ...
                threads = [
                    client.submit(
                        makes2DProjectionsFile, x, param, session1, dataFolder
                    )
            ...
            for index, fileName2Process in enumerate(param.fileList2Process):
                ...
                if (fileName == None) or (
                    ...
                    and (
                        ...
                        in [os.path.basename(x) for x in fileName]
                    )
                ):
                    makes2DProjectionsFile(
                        fileName2Process, param, session1, dataFolder
                    )
